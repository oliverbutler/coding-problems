...
export const step2 = (rawData: typeof data) => {
  ...
  const results = dataWithoutLastLine.map(row => {
    ...
    const howDoesRoundNeedToEnd = mapHowRoundNeedsToEnd[row[1] as "X"] as number

    const opp = mapOpponentToPiece[row[0] as "A"] as RockPaperScissors

    const findMyIdealPieceThatWillWin = ["ROCK", "PAPER", "SCISSORS"].find(piece => {
      const myScore = modelTheGame(opp, piece as RockPaperScissors)
      return myScore === howDoesRoundNeedToEnd
    })


    const myPieceShape = pieceShape[findMyIdealPieceThatWillWin as RockPaperScissors];
...
export const modelTheGame = (player1: RockPaperScissors, player2: RockPaperScissors): Status => {

  const gameSet = new Set([player1, player2]);
  const didPlayer2Win = (piece: RockPaperScissors) => player2 === piece ? 6 : 0;
