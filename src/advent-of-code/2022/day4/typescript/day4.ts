...
const doesOneRangeOverlapTheOther = (range1: [number, number], range2: [number, number]) => {
...
const doesOneRangeOverlapTheOtherAtAll = (range1: [number, number], range2: [number, number]) => {
...
export const step1 = (rawData: typeof data) => {
  ...
  const ranges = data.map(elfs => elfs.map(elf => elf.split('-').map(Number)));
  ...
  const rangesWhereTheresAnOverlap = ranges.filter(range => {
    ...
    return doesOneRangeOverlapTheOther(range1 as [number, number], range2 as [number, number]);
  })
...
export const step2 = (rawData: typeof data) => {
  ...
  const rangesWhereTheresAnOverlap = ranges.filter(range => {
    ...
    const [range1, range2] = range;
    return doesOneRangeOverlapTheOtherAtAll(range1 as [number, number], range2 as [number, number]);
  })
